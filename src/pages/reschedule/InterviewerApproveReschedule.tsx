...
const InterviewerApproveReschedule = () => {
  ...
  useEffect(() => {
    const handleRescheduleApproval = async () => {
      try {
        const { data } = await axiosInstance.put("/interviewer/approveReschedule", {
    ...
    if (requestId && interviewerId) {
      handleRescheduleApproval();
    } else {
      setStatus("error");
      setErrorMessage("Invalid request");
    }
  }, [requestId, interviewerId]);
  ...
  if (status === "loading") {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background space-y-4">
        ...
        >
          <p className="text-primary font-medium text-lg">Securing Your Time Slot</p>
        ...
        <AnimatePresence mode="wait">
          {status === "success" ? (
            ...
            >
              ...
              <h1 className="text-3xl font-bold text-foreground">Schedule Confirmed</h1>
              ...
              <div className="bg-success/10 p-4 rounded-lg flex items-center gap-3 text-success">
            ...
            >
              ...
              <h1 className="text-3xl font-bold text-primary">Action Required</h1>
              <p className="text-lg text-secondary font-medium">{errorMessage}</p>
...
const InfoItem = React.memo(
  ({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) => (
    <div className="flex items-center gap-4 text-left">
      <div className="bg-success/20 p-3 rounded-lg">{icon}</div>
      <div>
...
const ActionButton = React.memo(
  ({ onClick, icon, label }: { onClick: () => void; icon: React.ReactNode; label: string }) => (
    <button
      onClick={onClick}
      className="mt-6 bg-primary text-white hover:bg-primary/90 text-primary-foreground font-semibold py-3 px-8 rounded-full transition-colors flex items-center gap-2 mx-auto"
    >
      {icon}
      {label}
    </button>
...
export default InterviewerApproveReschedule;