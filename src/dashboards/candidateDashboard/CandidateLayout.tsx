...
const SidebarSubItem = memo(
  ({ label, isActive, onClick, isExpanded = true }: SidebarSubItemProps) => (
    <motion.button
      whileHover={{ x: 4 }}
      whileTap={{ x: 0 }}
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-2.5 pl-11 rounded-lg transition-all duration-200 text-sm ${
        isActive
...
const CandidateLayout: React.FC = () => {
  ...
  // Get the current tab heading based on the path
